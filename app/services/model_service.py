...
def generate_response(response, identifier, message_type=""):
    ...
    IS_USING_EXTERNAL = current_app.config['IS_USING_EXTERNAL']

    if IS_USING_API:
      if IS_USING_GPT:
        from ..services.gpt_service_api  import generate_response as model_generate_response
      elif IS_USING_EXTERNAL:
        from ..services.external_service_api  import generate_response as model_generate_response
      else:
